backgroundIMG = [
  "https://crownpointecommunities.com/wp-content/uploads/2017/06/sky-sunny-clouds-cloudy-scaled.jpg",
  "https://images.pexels.com/photos/2682122/pexels-photo-2682122.jpeg",
  "https://images.pexels.com/photos/391522/pexels-photo-391522.jpeg",
  "https://images.pexels.com/photos/436792/pexels-photo-436792.jpeg?cs=srgb&dl=pexels-dan-hamill-144328-436792.jpg&fm=jpg",
  "https://kubrick.htvapps.com/htv-prod-media.s3.amazonaws.com/images/noaa-iewhxjjaewy-unsplash-676f3dea13037.jpg?crop=1.00xw:0.846xh;0,0.0367xh&resize=1200:*",
  "https://images.squarespace-cdn.com/content/v1/63ceaffd33529a45e572bf90/1674497647090-7JRP5KMB2B80P3043HPA/image-asset.jpeg",
  "https://www.getinflow.io/cdn-cgi/image/fit=contain,format=auto,width=null/https://cdn.prod.website-files.com/623a0044a408ef57d0c818a3/651db0c5c04672d47996dc9f_less-stressed-nighttime.jpg",
  "https://guideposts.org/wp-content/uploads/2017/05/full_moon-1024x576.jpg.optimal.jpg",
];
let conthour = 0;
let srue = 0;
...
async function gettingWeather() {
  ...
  setInterval(() => {
    ...
    let fullDate = `${year}-${monthStr}-${dayStr}T${hourStr}:00`;
    if (a["hourly"]["time"].includes(fullDate) === true) {
      ...
      document.querySelector(
        "h1"
      ).innerText = `${a["hourly"]["temperature_2m"][indexTemp]}${a["hourly_units"]["temperature_2m"]}`;
      if (hour > 5 && hour < 21) {
        if (
          a["hourly"]["temperature_2m"][indexTemp] < 12 &&
          a["hourly"]["temperature_2m"][indexTemp] > 0
        ) {
          document.querySelector(
            "h4"
          ).innerText = `Feels chill at ${hour}:${minutes}`;
          document.querySelector(
            "body"
          ...
          document.querySelector(
            "h4"
          ).innerText = `Feels cold at ${hour}:${minutes}`;
          document.querySelector(
            "body"
          ).style.backgroundImage = `url(${backgroundIMG[3]})`;
        } else if (a["hourly"]["temperature_2m"][indexTemp] > 20) {
          document.querySelector(
            "h4"
          ).innerText = `Feels warm at ${hour}:${minutes}`;
          document.querySelector(
            "body"
          ...
          document.querySelector(
            "h4"
          ).innerText = `Feels okay at ${hour}:${minutes}`;
          document.querySelector(
            "body"
          ).style.backgroundImage = `url(${backgroundIMG[2]})`;
        }
      } else {
        if (
          a["hourly"]["temperature_2m"][indexTemp] < 12 &&
          ...
          document.querySelector(
            "body"
          ).style.backgroundImage = `url(${backgroundIMG[4]})`;
        } else if (a["hourly"]["temperature_2m"][indexTemp] < 0) {
          document.querySelector(
            "h4"
          ).innerText = `Feels cold at ${hour}:${minutes}`;
          document.querySelector(
            "body"
          ).style.backgroundImage = `url(${backgroundIMG[5]})`;
        } else if (a["hourly"]["temperature_2m"][indexTemp] > 20) {
          document.querySelector(
            "h4"
          ).innerText = `Feels warm at ${hour}:${minutes}`;
          document.querySelector(
            "body"
          ).style.backgroundImage = `url(${backgroundIMG[6]})`;
        } else {
          document.querySelector(
            "h4"
          ).innerText = `Feels okay at ${hour}:${minutes}`;
          document.querySelector(
            "body"
          ).style.backgroundImage = `url(${backgroundIMG[7]})`;
        }
      }
      const Wtoday = document.getElementById("Wtoday");
      while (hour + conthour <= 23) {
        if (
          a["hourly"]["time"].includes(
            `${year}-0${month}-${day}T${hour + conthour}:00`
          ) === true
        ) {
          srue++;
        }
            ...
            if (weekDayC === 1) {
              ...
              if (keepingCountt === -1) {
                console.log("hehe")
                    ...
                    keepingCounttNightindex.push(keepingCounttNight);
                    let tempchild = document.createElement("div");
                    console.log(a["hourly"]["temperature_2m"][keepCountindex[weekDayC]+24])
                    tempchild.innerHTML = tempchild.innerHTML = `<div>${
                      a["hourly"]["temperature_2m"][keepCountindex[weekDayC]+24]
                    } ${
                      a["hourly_units"]["temperature_2m"]
                    } <svg stroke="currentColor" fill="yellow" stroke-width="0" viewBox="0 0 24 24" height="2em" width="100%" xmlns="http://www.w3.org/2000/svg"><path fill="none" d="M0 0h24v24H0z"></path><path d="M11 4V2c0-.55.45-1 1-1s1 .45 1 1v2c0 .55-.45 1-1 1s-1-.45-1-1zm7.36 3.05 1.41-1.42a.996.996 0 1 0-1.41-1.41l-1.41 1.42a.996.996 0 1 0 1.41 1.41zM22 11h-2c-.55 0-1 .45-1 1s.45 1 1 1h2c.55 0 1-.45 1-1s-.45-1-1-1zm-10 8c-.55 0-1 .45-1 1v2c0 .55.45 1 1 1s1-.45 1-1v-2c0-.55-.45-1-1-1zM5.64 7.05 4.22 5.64c-.39-.39-.39-1.03 0-1.41s1.03-.39 1.41 0l1.41 1.41c.39.39.39 1.03 0 1.41s-1.02.39-1.4 0zm11.31 9.9a.996.996 0 0 0 0 1.41l1.41 1.41c.39.39 1.03.39 1.41 0a.996.996 0 0 0 0-1.41l-1.41-1.41a.996.996 0 0 0-1.41 0zM2 13h2c.55 0 1-.45 1-1s-.45-1-1-1H2c-.55 0-1 .45-1 1s.45 1 1 1zm3.64 6.78 1.41-1.41c.39-.39.39-1.03 0-1.41s-1.03-.39-1.41 0l-1.41 1.41a.996.996 0 0 0 0 1.41c.38.39 1.02.39 1.41 0zM12 6c-3.31 0-6 2.69-6 6s2.69 6 6 6 6-2.69 6-6-2.69-6-6-6z"></path></svg> </div><div>${
                      a["hourly"]["temperature_2m"][
                        keepingCounttNightindex[weekDayC]+24
                      ]
                    } ${
                      a["hourly_units"]["temperature_2m"]
                    } <svg stroke="currentColor" fill="currentColor" stroke-width="0" viewBox="0 0 512 512" height="1.5em" width="100%" xmlns="http://www.w3.org/2000/svg"><path d="M283.211 512c78.962 0 151.079-35.925 198.857-94.792 7.068-8.708-.639-21.43-11.562-19.35-124.203 23.654-238.262-71.576-238.262-196.954 0-72.222 38.662-138.635 101.498-174.394 9.686-5.512 7.25-20.197-3.756-22.23A258.156 258.156 0 0 0 283.211 0c-141.309 0-256 114.511-256 256 0 141.309 114.511 256 256 256z"></path></svg> </div>`;
                    ...
                    if (month.toString().length < 2) {
                      ...
                      if (newday.getDay()+ 1 === 7) {
                        ...
                        wp.innerHTML = `${daysOfTheWeek[newday.getDay()+1]}: <h5>0${
                        month
                      }-${day + weekDayC +1}</h5>`;
                      }
